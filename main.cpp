...
#define smaller_image_size 200 // result is stored in 100x100 matrices
...
bool remove_background(Mat &image)
{
	...
	for (int i = 0; i < image_width; i++)
		...
		flood_fill_background(image_height - motor_shaft_height - 1, i, &image, matrix, 1, image_width);

	// now I start from the center and fill the object
	// I did that because we can have multiple islands and only 1 is of interest
	flood_fill_fruit(image_height - motor_shaft_height - 30, image_width / 2, matrix, image_width);
	
	// ignore everything that is below shaft
	for (int i = image_height - motor_shaft_height; i < image_height; i++)
		for (int j = 0; j < image_width; j++)
...
int main(void)
{
	...
	// open video
	VideoCapture input_video(input_file_name + ".MOV");
